...
_RAW_ITEMS = [
    ...
    ("KISA-015", "적절한 인증 없는 중요 기능 허용", _CATEGORY_SECURITY_FEATURE, "2-1", "인증 절차 없이 중요 기능(관리자 기능 등)에 접근할 수 있는 취약점", "High", "IMPLEMENTED"),
    ("KISA-016", "부적절한 인가", _CATEGORY_SECURITY_FEATURE, "2-2", "권한 검증 없이 타인의 자원 또는 관리 기능에 접근할 수 있는 취약점", "High", "IMPLEMENTED"),
    ("KISA-017", "중요한 자원에 대한 잘못된 권한 설정", _CATEGORY_SECURITY_FEATURE, "2-3", "중요 자원(파일, DB 등)에 과도하게 허용적인 접근 권한이 설정된 취약점", "Medium", "IMPLEMENTED"),
    ...
    ("KISA-019", "암호화되지 않은 중요정보", _CATEGORY_SECURITY_FEATURE, "2-5", "개인정보 등 중요정보를 암호화하지 않고 저장·전송하는 취약점", "High", "IMPLEMENTED"),
    ...
    ("KISA-023", "취약한 비밀번호 허용", _CATEGORY_SECURITY_FEATURE, "2-9", "비밀번호 복잡도/길이 등 정책을 강제하지 않는 취약점", "Medium", "IMPLEMENTED"),
    ("KISA-024", "사용자 하드디스크에 저장되는 쿠키를 통한 정보노출", _CATEGORY_SECURITY_FEATURE, "2-10", "중요정보를 영구 쿠키 등 클라이언트 저장소에 평문 저장하는 취약점", "Medium", "IMPLEMENTED"),
    ("KISA-025", "주석문 안에 포함된 시스템 주요정보", _CATEGORY_SECURITY_FEATURE, "2-11", "소스코드 주석에 계정, 내부 경로 등 민감정보가 노출되는 취약점", "Low", "IMPLEMENTED"),
    ("KISA-026", "솔트 없이 일방향 해시함수 사용", _CATEGORY_SECURITY_FEATURE, "2-12", "비밀번호 해시 시 salt를 사용하지 않아 레인보우 테이블 공격에 취약한 문제", "Medium", "IMPLEMENTED"),
    ("KISA-027", "경쟁조건: 검사 시점과 사용 시점(TOCTOU)", _CATEGORY_TIME_STATE, "3-1", "자원 상태를 검사한 시점과 실제 사용하는 시점 사이의 경쟁조건을 이용한 취약점", "Medium", "IMPLEMENTED"),
    ("KISA-028", "종료되지 않는 반복문 또는 재귀함수", _CATEGORY_TIME_STATE, "3-2", "종료 조건이 보장되지 않아 자원 고갈을 유발할 수 있는 반복/재귀 구조", "Low", "IMPLEMENTED"),
    ("KISA-029", "오류 메시지를 통한 정보노출", _CATEGORY_ERROR, "4-1", "예외 발생 시 스택 트레이스 등 시스템 내부 정보가 사용자에게 노출되는 취약점", "Medium", "IMPLEMENTED"),
    ("KISA-030", "오류 상황 대응 부재", _CATEGORY_ERROR, "4-2", "예외적인 상황(널값, 자원 부족 등)에 대한 처리가 누락된 취약점", "Medium", "IMPLEMENTED"),
    ("KISA-031", "부적절한 예외처리", _CATEGORY_ERROR, "4-3", "여러 예외를 하나로 뭉뚱그려 처리하거나 예외 유형을 지나치게 광범위하게 처리하는 문제", "Low", "IMPLEMENTED"),
    ("KISA-032", "빈 Catch 블록", _CATEGORY_ERROR, "4-4", "예외를 포착하고도 아무 처리 없이 무시하여 오류 상황이 은폐되는 취약점", "Low", "IMPLEMENTED"),
    ("KISA-033", "널 포인터 역참조", _CATEGORY_CODE_ERROR, "5-1", "널 값 가능성이 있는 참조를 검증 없이 역참조하여 예외가 발생하는 결함", "Medium", "IMPLEMENTED"),
    ("KISA-034", "부적절한 자원 해제", _CATEGORY_CODE_ERROR, "5-2", "파일 핸들, 커넥션 등 자원을 명시적으로 해제하지 않아 고갈을 유발하는 결함", "Medium", "IMPLEMENTED"),
    ("KISA-035", "해제된 자원 사용", _CATEGORY_CODE_ERROR, "5-3", "이미 해제된 자원을 재사용하는 결함(Use-After-Free 계열)", "Medium", "IMPLEMENTED"),
    ...
    ("KISA-043", "시스템 데이터 정보노출", _CATEGORY_ENCAPSULATION, "6-3", "내부 시스템 구조/설정 정보가 외부에 노출되는 취약점", "Medium", "IMPLEMENTED"),
    ("KISA-044", "Public 메소드로부터 반환된 Private 배열", _CATEGORY_ENCAPSULATION, "6-4", "내부 배열을 참조로 반환하여 외부에서 내부 상태를 임의 변경할 수 있는 캡슐화 결함", "Low", "IMPLEMENTED"),
    ("KISA-045", "Private 배열에 Public 데이터 할당", _CATEGORY_ENCAPSULATION, "6-5", "외부에서 전달받은 배열 참조를 그대로 내부 상태에 저장하는 캡슐화 결함", "Low", "IMPLEMENTED"),
    ...
    ("KISA-047", "DNS Lookup에 의존한 보안 결정", _CATEGORY_API_MISUSE, "7-1", "DNS 조회 결과(호스트명 등)를 신뢰하여 보안 결정을 내리는 취약점", "Low", "IMPLEMENTED"),
    ("KISA-048", "취약한 API 사용", _CATEGORY_API_MISUSE, "7-2", "안전하지 않거나 지원 종료된 API를 사용하는 취약점", "Low", "IMPLEMENTED"),
    ("KISA-049", "운영체제 명령 실행 API 오용", _CATEGORY_API_MISUSE, "7-3", "운영체제 명령 실행 API를 목적에 맞지 않게 오용하는 취약점", "Medium", "IMPLEMENTED"),
]
