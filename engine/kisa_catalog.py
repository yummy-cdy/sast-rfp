...
_RAW_ITEMS = [
    ...
    ("KISA-005", "위험한 형식 파일 업로드", _CATEGORY_INPUT, "1-5", "실행 가능한 스크립트 파일의 업로드를 제한하지 않아 서버 측 코드가 실행될 수 있는 취약점", "High", "PLANNED"),
    ...
    ("KISA-007", "XQuery 삽입", _CATEGORY_INPUT, "1-7", "검증되지 않은 외부 입력값이 XQuery 쿼리에 삽입되는 취약점", "Medium", "PLANNED"),
    ("KISA-008", "XPath 삽입", _CATEGORY_INPUT, "1-8", "검증되지 않은 외부 입력값이 XPath 표현식에 삽입되는 취약점", "Medium", "PLANNED"),
    ("KISA-009", "LDAP 삽입", _CATEGORY_INPUT, "1-9", "검증되지 않은 외부 입력값이 LDAP 쿼리에 삽입되는 취약점", "Medium", "PLANNED"),
    ("KISA-010", "크로스사이트 요청 위조(CSRF)", _CATEGORY_INPUT, "1-10", "정상 사용자의 세션을 이용해 의도되지 않은 요청이 처리되는 취약점", "High", "PLANNED"),
    ("KISA-011", "서버사이드 요청 위조(SSRF)", _CATEGORY_INPUT, "1-11", "검증되지 않은 외부 입력값으로 서버가 임의 주소에 요청을 보내는 취약점", "High", "IMPLEMENTED"),
    ("KISA-012", "HTTP 응답분할", _CATEGORY_INPUT, "1-12", "검증되지 않은 외부 입력값이 HTTP 응답 헤더에 삽입되는 취약점", "Medium", "PLANNED"),
    ("KISA-013", "정수형 오버플로우", _CATEGORY_INPUT, "1-13", "정수 연산 결과가 자료형의 표현 범위를 초과하여 예기치 않은 동작을 유발하는 취약점", "Medium", "PLANNED"),
